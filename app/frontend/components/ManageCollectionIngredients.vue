    ...
    <div>
      ...
      <div v-if="filteredAvailableIngredients.length > 0" class="flex flex-wrap gap-2">
        <button
          ...
          type="button"
          :disabled="pendingIds.has(ingredient.id)"
          class="px-3 py-2 bg-cs-ink-100 text-cs-ink-700 rounded-lg hover:bg-cs-ink-200 transition text-sm font-medium flex items-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed"
        >
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-4 h-4">
