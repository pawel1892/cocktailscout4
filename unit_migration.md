...
#### Rendering
Der Nutzer bekommt nur die Beschreibung zu sehen. Menge und Zutat werden intern verwendet, um Gesamtvolumen und Alkoholgehalt zu berechnen, sowie für „Meine Bar" und andere Suchmöglichkeiten nach Zutaten.
...
#### Edge Cases
Der einfachste und am häufigsten vorkommende Fall ist, dass die Beschreibung mit den internen Daten übereinstimmt. Beispiel:
`amount: 2, ingredient: Tequila (weiss), description: 2 cl Tequila (weiss)`

In allen anderen Fällen, wo die Beschreibung nicht mit den internen Daten übereinstimmt, haben wir zwei Wahrheiten und müssen uns für eine entscheiden. Das ist schwierig zu automatisieren und muss manuell nachbearbeitet werden.
...
Zusätzlich dazu gibt es natürlich alle Standardfelder: id, timestamps.

### Tabelle: recipe_ingredients
#### Neue Felder
- `amount` decimal(10,2) DEFAULT NULL: Menge in der neuen Einheit (unit_id)
- `unit_id` bigint(20) DEFAULT NULL: Einheit (Relation zu units)
...
- `is_optional` tinyint(1) NOT NULL DEFAULT 0: Im Wesentlichen für „Meine Bar" (kann diese Zutat weggelassen werden?)
- `display_name` varchar(255) DEFAULT NULL: Falls ein anderer Zutatenname angezeigt werden muss als der in der Zutatentabelle. Beispiel: Zutat: Erdbeeren – Display Name: gefrostete Erdbeeren

### Tabelle: ingredients
Neu: `:plural_name, :string, after: :name`

Das wird benötigt, um beim Skalieren „1 Limette" und „2 Limetten" im Plural anzuzeigen.

#### Neue Relation
Rezeptzutaten (recipe_ingredients) haben eine Einheit (units).
...
Hierfür muss die `old_description` geparst werden. Es muss versucht werden, in diesem String ein Muster zu finden: Menge + Einheit + Zutat oder Menge + Zutat. In Rezeptzutaten ist die Einheit optional, da es Dinge wie Limetten oder Minzzweige gibt. Diese haben die implizite Einheit Stück.

Beispiele:
- `"2 cl Wodka"` → Menge: 2, Einheit: cl, Zutat: Wodka
- `"ein Spritzer Cola"` → Menge: 1, Einheit: Spritzer, Zutat: Cola
...
Um keine Informationen zu verlieren, müssen wir nach dem Parsen auch überprüfen, ob der Zutatenname in `old_description` und der Name der Zutat in der Zutatentabelle (ingredients) übereinstimmen. Es geht nicht darum, ob die Strings zu 100% übereinstimmen, sondern ob es inhaltlich übereinstimmt. Das lässt sich natürlich nicht per Skript automatisieren. Es gibt aber ein Übersetzungsarray:
...
Wenn die Zutaten nach Überprüfung des Arrays nicht übereinstimmen, wird ebenfalls das `needs_review`-Flag gesetzt, um eine manuelle Überprüfung zu triggern.
...
Hier würden die Zusatzinformationen (gefrostet, nur Eigelb, Marke Wyborowa) bei einer automatischen Migration verloren gehen.

Wenn `needs_review` gesetzt ist, zeigt das System dem Nutzer die `old_description`. Diese Rezepte können nicht nach Portionen skaliert werden, bis sie bereinigt wurden.
